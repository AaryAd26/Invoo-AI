...
// check the string is object id or not 
...
// generate unique invoice number to avaiod collision
...
export async function getInvoices(req, res) {
    ...
    const q = {owner: userId}; // FIXED: was userID
...
// GET INVOICE BY ID
export async function getInvoiceById(req, res){ // FIXED: was getInvoicesById
    try{
        const {userId} = getAuth(req) || {}; 
        if (!userId){
            return res.status(401).json({
        ...
        else inv = await Invoice.findOne({invoiceNumber: id}); // FIXED: was invoice
        ...
        if (inv.owner && String(inv.owner) !== String(userId)){ // FIXED: was string()
            return res.status(403).json({
                success: false,
                message: "Forbidden"
            });
        }
        return res.status(200).json({ // FIXED: was res.Status, moved outside if block
            success: true,
            data: inv
        });
    }
