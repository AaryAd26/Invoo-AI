...
// status with count
export  function StatusWithCount({status, count, size="defult"}) {
    return(
        <div className= " inline-flex items-center gap-2 group">
            < StatusBadge status = {status} size={size}/>
            {count !== undefined && (
                <span className="text-xs text-grey-500 font-medium bg-grey-100 px-2 py-1 rounded-full group-hover:bg-gray-200 transition-colors duration-200"
                > {count}
                </span>
            )}
        </div>
