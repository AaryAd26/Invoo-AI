...
export default function CreateInvoice() {
  ...
  function removeImage(kind = "logo") {
    setInvoice((inv) =>
      inv ? { ...inv, [`${kind}DataUrl`]: null } : inv
    );
  }

  /* ---------- FIX 1: checkInvoiceExists — /api/invoice → /api/invoices ---------- */
  ...
  /* ---------- FIX 2: fetchBusinessProfile — /api/businessProfile → /api/business-profile ---------- */
  useEffect(() => {
    let mounted = true;
    async function fetchBusinessProfile() {
      ...
      try {
        ...
        setInvoice((prev) => {
          ...
          return {
            ...prev,
            fromBusinessName: !prev.fromBusinessName?.trim()
              ? serverProfile.businessName
              : prev.fromBusinessName,
            fromEmail: !prev.fromEmail?.trim()
              ? serverProfile.email
              : prev.fromEmail,
            fromAddress: !prev.fromAddress?.trim()
              ? serverProfile.address
              : prev.fromAddress,
            fromPhone: !prev.fromPhone?.trim()
              ? serverProfile.phone
              : prev.fromPhone,
            fromGst: !prev.fromGst?.trim()
              ? serverProfile.gst
              : prev.fromGst,
            logoDataUrl:
              prev.logoDataUrl ||
  ...
  }, [isSignedIn, isLoaded, obtainToken]);

  /* ---------- FIX 3: load invoice when editing — /api/invoice/:id → /api/invoices/:id ---------- */
  useEffect(() => {
    let mounted = true;
    async function prepare() {
      if (invoiceFromState) {
        ...
        base.logoDataUrl =
          resolveImageUrl(base.logoDataUrl ?? base.logoUrl ?? base.logo) || null;
        base.stampDataUrl =
          resolveImageUrl(base.stampDataUrl ?? base.stampUrl ?? base.stamp) || null;
        base.signatureDataUrl =
          resolveImageUrl(base.signatureDataUrl ?? base.signatureUrl ?? base.signature) || null;
        setInvoice(base);
        setItems(
      ...
      if (isEditing && !invoiceFromState) {
        ...
        try {
          ...
          if (res.ok) {
            ...
            if (data && mounted) {
              ...
              merged.logoDataUrl =
                resolveImageUrl(data.logoDataUrl ?? data.logoUrl ?? data.logo) ||
                merged.logoDataUrl || null;
              merged.stampDataUrl =
                resolveImageUrl(data.stampDataUrl ?? data.stampUrl ?? data.stamp) ||
                merged.stampDataUrl || null;
              merged.signatureDataUrl =
                resolveImageUrl(data.signatureDataUrl ?? data.signatureUrl ?? data.signature) ||
          ...
          console.warn("Server invoice fetch failed, falling back to local:", err);
        } finally {
          setLoading(false);
        }
        ...
        if (found && mounted) {
          ...
          fixed.logoDataUrl =
            ...
            fixed.logoDataUrl || null;
          fixed.stampDataUrl =
            resolveImageUrl(found.stampDataUrl ?? found.stampUrl ?? found.stamp) ||
            fixed.stampDataUrl || null;
          fixed.signatureDataUrl =
            resolveImageUrl(found.signatureDataUrl ?? found.signatureUrl ?? found.signature) ||
            fixed.signatureDataUrl || null;
          setInvoice(fixed);
          setItems(
            Array.isArray(found.items) ? found.items.slice() : buildDefaultInvoice().items
          );
        }
        return;
      }
      ...
      if (!isEditing) {
        try {
          ...
          if (mounted) {
            setInvoice((inv) => inv ? { ...inv, invoiceNumber: candidate } : inv);
          }
        } catch (err) {
  ...
  /* ---------- FIX 4: handleSave — /api/invoice → /api/invoices ---------- */
  async function handleSave() {
    ...
    try {
      ...
      // ✅ FIXED: /api/invoice → /api/invoices
      const endpoint =
          ...
          <div className={createInvoiceStyles.cardContainer}>
            ...
            <div className="mt-6">
              <button onClick={addItem} className={createInvoiceStyles.addItemButton}>
                <AddIcon className={`w-4 h-4 ${createInvoiceStyles.iconHover}`} /> Add Item
              </button>
            </div>
          </div>
        </div>
                  ...
                  {invoice?.logoDataUrl ? (
                    <div className={createInvoiceStyles.imagePreviewContainer}>
                      <div className={createInvoiceStyles.logoPreview}>
                        <img src={invoice.logoDataUrl} alt="logo" className="object-contain w-full h-full" />
                      </div>
                      <div className={createInvoiceStyles.buttonGroup}>
                        ...
                        <button onClick={() => removeImage("logo")} className={createInvoiceStyles.removeButton}>
                    ...
                    <label className="cursor-pointer block">
                      <div className={`${createInvoiceStyles.imagePreviewContainer} ${createInvoiceStyles.hoverScale}`}>
                        ...
                        <div>
                          <p className={createInvoiceStyles.uploadTextTitle}>Upload Logo</p>
                          <p className={createInvoiceStyles.uploadTextSubtitle}>PNG, JPG up to 5MB</p>
                        </div>
                        <input type="file" accept="image/*" onChange={(e) => handleImageUpload(e.target.files?.[0], "logo")} className="hidden" />
                      </div>
                    </label>
              ...
              <div className="space-y-3">
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => updateInvoiceField("taxPercent", Number(e.target.value || 0))}
            ...
            <div className="space-y-6">
                  ...
                  {invoice.stampDataUrl ? (
                    <div className={createInvoiceStyles.imagePreviewContainer}>
                      <div className={createInvoiceStyles.stampPreview}>
                        <img src={invoice.stampDataUrl} alt="stamp" className="object-contain w-full h-full" />
                      </div>
                      <div className={createInvoiceStyles.buttonGroup}>
                        <label className={createInvoiceStyles.changeButton}>
                          <UploadIcon className="w-4 h-4" /> Change
                        ...
                        <button onClick={() => removeImage("stamp")} className={createInvoiceStyles.removeButton}>
                          <DeleteIcon className="w-4 h-4" /> Remove
                        </button>
                      </div>
                    </div>
                    ...
                    <label className="cursor-pointer block">
                      <div className={`${createInvoiceStyles.imagePreviewContainer} ${createInvoiceStyles.hoverScale}`}>
                        ...
                        <div>
                          <p className={createInvoiceStyles.uploadTextTitle}>Upload Stamp</p>
                          <p className={createInvoiceStyles.uploadTextSubtitle}>PNG with transparency</p>
                        </div>
                        <input type="file" accept="image/*" onChange={(e) => handleImageUpload(e.target.files?.[0], "stamp")} className="hidden" />
                      </div>
                    </label>
              ...
              <div>
                  ...
                  {invoice.signatureDataUrl ? (
                    <div className={createInvoiceStyles.imagePreviewContainer}>
                      <div className={createInvoiceStyles.signaturePreview}>
                        <img src={invoice.signatureDataUrl} alt="signature" className="object-contain w-full h-full" />
                      </div>
                      <div className={createInvoiceStyles.buttonGroup}>
                        <label className={createInvoiceStyles.changeButton}>
                          <UploadIcon className="w-4 h-4" /> Change
                        ...
                        <button onClick={() => removeImage("signature")} className={createInvoiceStyles.removeButton}>
                          <DeleteIcon className="w-4 h-4" /> Remove
                        </button>
                      </div>
                    </div>
                    ...
                    <label className="cursor-pointer block">
                      <div className={`${createInvoiceStyles.imagePreviewContainer} ${createInvoiceStyles.hoverScale}`}>
                        ...
                        <div>
                          <p className={createInvoiceStyles.uploadTextTitle}>Upload Signature</p>
                          <p className={createInvoiceStyles.uploadTextSubtitle}>PNG with transparency</p>
                        </div>
                        <input type="file" accept="image/*" onChange={(e) => handleImageUpload(e.target.files?.[0], "signature")} className="hidden" />
                      </div>
                ...
                <div className="mt-4 space-y-3">
                  <div>
                    <label className={createInvoiceStyles.label}>Signature Owner Name</label>
                    <input
                      placeholder="John Doe"
                      value={invoice.signatureName || ""}
                      onChange={(e) => updateInvoiceField("signatureName", e.target.value)}
                      className={`${createInvoiceStyles.inputSmall} ${createInvoiceCustomStyles.inputPlaceholder}`}
                    />
                  </div>
                  <div>
                    <label className={createInvoiceStyles.label}>Signature Title / Designation</label>
                    <input
                      placeholder="Director / CEO"
                      value={invoice.signatureTitle || ""}
                      onChange={(e) => updateInvoiceField("signatureTitle", e.target.value)}
                      className={`${createInvoiceStyles.inputSmall} ${createInvoiceCustomStyles.inputPlaceholder}`}
                    />
                  </div>
                </div>
